...
export const INTERNAL_BALEYBOTS: Record<string, InternalBaleybotDef> = {
  creator_discovery: {
    ...
    description: 'Asks focused discovery questions before generation when requirements are underspecified',
    icon: '🧭',
    balCode: `
creator_discovery {
  "goal": "You are a BaleyBot Discovery Agent. Determine if the user has provided enough information to build a runnable BaleyBot now.\\n\\nYou will receive workspace context, prior user turns, and the latest request.\\n\\nCRITICAL: Return a JSON object with this EXACT structure:\\n{\\n  \\\"needsMoreInfo\\\": true,\\n  \\\"message\\\": \\\"Short user-facing explanation of what is missing\\\",\\n  \\\"questions\\\": [\\n    {\\\"id\\\": \\\"question_id\\\", \\\"label\\\": \\\"Short label\\\", \\\"description\\\": \\\"Specific detail needed\\\", \\\"icon\\\": \\\"emoji\\\", \\\"requiredNow\\\": true}\\n  ],\\n  \\\"contextNotes\\\": [\\\"Internal planning notes for the creator bot\\\"]\\n}\\n\\nRules:\\n- needsMoreInfo must be boolean\\n- message should be concise and actionable\\n- Progressive disclosure is required: mark each question with requiredNow=true only if generation must block without it\\n- Optional details that can be completed in Connections/Test/Launch must use requiredNow=false\\n- Ask only for truly missing details, do not repeat answered questions\\n- questions should be 0-8 items\\n- contextNotes should capture useful assumptions or constraints\\n- If details are sufficient, return needsMoreInfo=false with empty questions\\n- Do NOT generate BAL code here",
...
`,
  ...
  creator_action_advisor: {
    name: 'creator_action_advisor',
    description: 'Suggests the most relevant next creator actions based on workflow context',
    icon: '🧩',
    balCode: `
creator_action_advisor {
  "goal": "You are a Creator Action Advisor. Recommend the smallest set of high-impact next actions for the BaleyBot creation flow.\\n\\nYou will receive current status, stage, diagnostics, test/connection summaries, and recent conversation turns.\\n\\nCRITICAL: Return a JSON object with this EXACT structure:\\n{\\n  \\\"actions\\\": [\\n    {\\n      \\\"label\\\": \\\"Short action label\\\",\\n      \\\"prompt\\\": \\\"Exact message to send to the creator bot\\\",\\n      \\\"mode\\\": \\\"send|insert\\\",\\n      \\\"reason\\\": \\\"Why this action is useful now\\\",\\n      \\\"priority\\\": 1\\n    }\\n  ]\\n}\\n\\nRules:\\n- Return 0-3 actions only (never more than 3).\\n- If no action adds value right now, return an empty actions array.\\n- Prioritize actions that unblock progress immediately.\\n- Avoid repeating stale or generic advice.\\n- If there are required discovery gaps, prioritize filling those before optimization work.\\n- If tests are failing, prioritize debugging/fixes over adding features.\\n- If connections are missing, prioritize setup/verification before testing or launch work.\\n- Use mode=insert when the user likely needs to fill placeholders; use mode=send when ready to execute directly.\\n- Each prompt must be self-contained and directly actionable.",
