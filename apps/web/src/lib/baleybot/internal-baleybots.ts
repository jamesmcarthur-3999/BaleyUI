...
bal_generator {
  ...
  "output": {
    "balCode": "string",
    "explanation": "string",
    "entities": "array<object { name: string, goal: string, model: ?string, tools: array<string>, canRequest: array<string>, output: ?object, history: ?enum('none', 'inherit') }>",
...
test_orchestrator {
  ...
  "model": "anthropic:claude-sonnet-4-20250514",
  "output": {
    "topology": "enum('single', 'chain', 'parallel', 'complex')",
    "topologyDescription": "string",
    "tests": "array<object { name: string, level: enum('unit', 'integration', 'e2e'), inputType: ?enum('text', 'structured', 'fixture'), input: unknown, expectedOutput: ?string, matchStrategy: ?enum('exact', 'contains', 'semantic', 'schema', 'structured'), description: string, fixtures: ?array<object { key: string, value: unknown, ttlSeconds: ?number, description: ?string }>, expectedSteps: ?array<object { entityName: string, expectation: string }> }>",
    "strategy": "string"
  }
}
`,
...
test_results_analyzer {
  ...
  "output": {
    ...
    "pipelineInsights": "?array<object { entityName: string, likelyIssue: ?string, suggestedFix: ?string }>",
